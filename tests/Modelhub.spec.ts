...
await page.getByRole('link', { name: 'Model Hub' }).click();
  await page.getByRole('button', { name: 'Try out' }).first().click();
  await page.getByRole('textbox', { name: 'Enter query' }).click();
  await page.getByRole('textbox', { name: 'Enter query' }).fill('What is Use Of MCP Server?');
